class Curves {
  ...
  getPoints() {
    const listPrefix = this.points[0][0] !== 0 ? [[0, this.points[0][1]]] : [];
    const listSuffix =
  ...
  onMouseDown(e) {
    if (this.selectedPoint !== null) {
      ...
      const nextX =
        this.points[this.selectedPointIndex + 1]?.[0] ?? this.svgRect.width;
      const onMouseMove = (e) => {
        let actualX = e.clientX - this.svgRect.left;
        let actualY = e.clientY - this.svgRect.top;
        ...
        y = Math.max(0, Math.min(this.svgRect.height / this.svgRect.height, y));
